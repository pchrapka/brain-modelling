...
p = inputParser;
addParameter(p,'type','all',@(x) any(validatestring(x,{'all','outer','outer-nocerebellum'})));
addParameter(p,'verbosity',0);
parse(p,varargin{:});

flag_outer = false;
flag_nocerebellum = false;

switch p.Results.type
    case 'all'
    case 'outer'
        flag_outer = true;
    case 'outer-nocerebellum'
        flag_outer = true;
        flag_nocerebellum = true;
    otherwise
        error('add rules for %s',p.Results.type);
end
