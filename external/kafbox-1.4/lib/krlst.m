        ...
        function [mean_test, var_test] = evaluate(kaf,x) % evaluate
            if size(kaf.dict,1)>0
                ...
                if nargout>1
                    ...
                    % predict
                    kxx = kernel(x,x,kaf.kerneltype,kaf.kernelpar);
                    ktt = kxx + kaf.jitter;
                    gamma2 = ktt - k'*q; gamma2(gamma2<0)=0; % projection uncertainty
                    h = kaf.Sigma*q;
                    sf2 = gamma2 + q'*h; sf2(sf2<0)=0; % noiseless prediction variance
                    sy2 = kaf.sn2 + sf2;
                    var_test = kaf.s02*sy2; % predictive variance
                end
            else
                mean_test = zeros(size(x,1),1); % prior
                var_test = kaf.s02*kaf.sn2; % prior
            end
        end
