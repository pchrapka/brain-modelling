% exp19_krls_anomaly_eeg_time
% KRLS-T anomaly detection algorithm for EEG data
%
% Goal:
%   Same as exp19_krls_anomaly_eeg except time is added to the input vector
%
% Result:
...
for j=1:ntrials_test%ntrials
    ...
    trial_data(:,~idx_time) = [];
    nchannels = size(trial_data,1);
    trial_data(nchannels+1,:) = data_std.sourceanalysis_all(j).time(idx_time);
...
for j=1:ntrials_test%ntrials
    ...
    trial_data(:,~idx_time) = [];
    nchannels = size(trial_data,1);
    trial_data(nchannels+1,:) = data_odd.sourceanalysis_all(j).time(idx_time);
