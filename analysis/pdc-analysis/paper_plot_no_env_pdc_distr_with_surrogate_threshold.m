%% paper_plot_no_env_pdc_distr_with_surrogate_threshold
...
dir_data = fullfile(dir_root,'output','std-s03-10',...
    ...
    'lf-data-trialsall-samplesall-normeachchannel-envno-prependflipdata');
...
idx_channel_in = 6; % auditory
idx_channel_out = 1; % temporal
idx_sample = 656;
% idx_sample = 300;

plot_params = [];
pdc_params = 'pdc-dynamic-diag-f2048-81-ds4';

k=1;
plot_params(k).stub_filter = 'MCMTLOCCD_TWL4-T20-C7-P6-lambda0.9900-gamma1.000e-05';
plot_params(k).perm_stat = '-p1';
plot_params(k).perm_surrogate = '-p1';
plot_params(k).slug_name = 'noenv-trials20';

k = k+1;

plot_params(k).stub_filter = 'MCMTLOCCD_TWL4-T100-C7-P6-lambda0.9900-gamma1.000e-05';
plot_params(k).perm_stat = '-p1';
plot_params(k).perm_surrogate = '-p1';
plot_params(k).slug_name = 'noenv-trials100';
k = k+1;
...
for k=1:length(plot_params)
    ...
    plot_pdc_distr_with_surrogate_threshold(file_name_std, file_name_mean,...
        ...
        ...'surrogate_files', {file_name_surrogate_ind},...
        ...'surrogate_legend', {'threshold - no coupling'},...
        'freq_step', 0.5,...
        'freq_max', 40,...
