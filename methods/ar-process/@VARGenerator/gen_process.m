...
addParameter(p,'mu',zeros(obj.nchannels,1),@isnumeric);
...
while j <= p.Results.ntrials
    fprintf('simulating\n')
    % simulate process
    [signal,signal_norm,~] = process.simulate(...
        p.Results.nsamples,p.Results.mu,p.Results.sigma);
    
    data.mu = p.Results.mu;
    data.sigma = p.Results.sigma;
