function Vout = interp1_nosort2(varargin)
...
%
%   PP = INTERP1(X,V,METHOD,'pp') uses the interpolation algorithm specified
...
if any(~isfinite(V(:))) ...
    ...
    if any(strcmpi(F.Method,{'spline','pchip'})) && any(isnan(V(:)))
...
if isempty(method)
    methodname = 'linear';
    warning(message('MATLAB:interp1:EmptyMethod'));
else
    if method(1) == '*'
        method(1) = [];
    end
...
%         end
        ...
        warning(message('MATLAB:interp1:ppGriddedInterpolant'));
        return
