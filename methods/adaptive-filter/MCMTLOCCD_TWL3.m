classdef MCMTLOCCD_TWL3
    %MCMTLOCCD_TWL3 Multichannel Lattice Online Cyclic Coordinate Descent Time Weighted Lasso
    ...
    methods
        function obj = MCMTLOCCD_TWL3(channels,order,trials,varargin)
            %MCMTLOCCD_TWL3
            %   MCMTLOCCD_TWL3(channels,order,...)
            %   
            %   Inputs
            %   ------
            %   channels
        ...
        function obj = update(obj, y, varargin)
            ...
            for m=2:obj.order+1
                ...
                for t=1:obj.ntrials
                    if t == 1
                        lambda_temp = obj.lambda;
                    else
                        lambda_temp = 1;
                    end
                    Rb_new = lambda_temp*squeeze(obj.Rb(m-1,:,:)) + obj.berrord(:,t,m-1)*obj.berrord(:,t,m-1)';
                    Rf_new = lambda_temp*squeeze(obj.Rf(m-1,:,:)) + ferror(:,t,m-1)*ferror(:,t,m-1)';
                    
                    Rbf_new = lambda_temp*squeeze(obj.Rbf(m-1,:,:)) + obj.berrord(:,t,m-1)*ferror(:,t,m-1)';
                    Rfb_new = lambda_temp*squeeze(obj.Rfb(m-1,:,:)) + ferror(:,t,m-1)*obj.berrord(:,t,m-1)';
