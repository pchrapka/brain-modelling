...
[expdir,~,ext] = fileparts(exp_path);
if isempty(ext)
    error('missing file name in exp_path');
end
...
if p.Results.plot_avg_mse
    ...
    plot_mse_vs_iteration(...
        ...
        'labels',labels);
    drawnow;
    save_fig_exp(exp_path,'tag',sprintf('mse-all-%s',tag));
    
    ylim([10^(-4) 10^(0)]);
    drawnow;
    save_fig_exp(exp_path,'tag',sprintf('mse-all-axis-%s',tag));
end
...
if p.Results.plot_avg_nmse
    ...
    plot_mse_vs_iteration(...
        ...
        'labels',labels);
    drawnow;
    save_fig_exp(exp_path,'tag',sprintf('nmse-all-%s',tag));
    
    ylim([10^(-1) 10^(3)]);
    drawnow;
    save_fig_exp(exp_path,'tag',sprintf('nmse-all-axis-%s',tag));
end
